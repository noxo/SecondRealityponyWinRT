...
namespace SecondRealipony
{
    class Intro : SRSegment
    {
        ...
        public override string MusicName { get { return "intro"; } }
            ...
            : base(game)
        {
            ...
            landscape = game.Content.Load<Texture2D>("intro");
            dash = game.Content.Load<Texture2D>("intro dash");
            spitfire = game.Content.Load<Texture2D>("wonderbolt1");
            soarin = game.Content.Load<Texture2D>("wonderbolt2");
            mistyfly = game.Content.Load<Texture2D>("wonderbolt3");
            ...
            bookman = game.Content.Load<SpriteFont>("calibri");
            texts = new IntroText[] {
                new IntroText(1, 15,
        ...
        private void CreateRaindrops()
        {
            ...
            for (int i = 0; i < raindrops; i++)
            {
                ...
                var contourColored = contour.Select(v =>
                    new VertexPositionColor(v.Position, HSLtoRGB(new ColorHSL(((i * 720F / raindrops) + hueOffset) % 360, 1.0F, v.Color.Color.A / 255F, v.Color.Color.A)))
                    ).ToArray();
