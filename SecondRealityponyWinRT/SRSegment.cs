...
namespace SecondRealipony
{
    abstract class SRSegment
    {
        ...
        public bool IsReady
        {
            get
            {
                return StartupThread == null || StartupThread.IsCompleted;
            }
        }
        ...
        {
            if (StartupThread != null && !StartupThread.IsCompleted)
                cts.Cancel();
        }
        ...
        public void CreateIndices(int width, int height, out int[] indices, out IndexBuffer indexBuffer)
        {
            ...
            indexBuffer = new IndexBuffer(
                device,
#if SURFACE_RT
                IndexElementSize.SixteenBits, // should optimize content also
#else
